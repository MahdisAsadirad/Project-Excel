...
import static org.example.controller.ExpressionParser.isFillCommand;
...
public class CommandProcessor {
    ...
    private void processFillCommand(String command) {
        try {
            System.out.println("DEBUG: Processing Fill command: " + command);

            // استفاده از ExpressionParser برای تجزیه دستور
            String[] fillParams = ExpressionParser.parseFillCommand(command);
            String sourceCell = fillParams[0];
            String targetRange = fillParams[1];

            System.out.println("DEBUG: Source: " + sourceCell + ", Target: " + targetRange);
            ...
            // انجام AutoFill
            autoFillManager.autoFill(command);
    ...
    // در CommandProcessor - متد displayHelp
    private void displayHelp() {
        System.out.println("\n=== AVAILABLE COMMANDS ===");
        System.out.println("<cell>=<value>           - Set cell content (e.g., A1=5, B1=\"Hello\", C1=A1+B1)");
        System.out.println("Fill(source, range)      - AutoFill from source cell to target range (e.g., Fill(A1, A1:C1))");
    ...
    private boolean isFillCommand(String command) {
        if (command == null) return false;
        String upperCommand = command.toUpperCase().trim();

        // فرمت‌های مختلف دستور Fill
        return upperCommand.matches("^FILL\\s*\\(\\s*[A-Za-z]\\d+\\s*,\\s*[A-Za-z]\\d+\\s*:\\s*[A-Za-z]\\d+\\s*\\)$") ||
                upperCommand.matches("^FILL\\s*\\(\\s*[A-Za-z]\\d+\\s*,\\s*[A-Za-z]\\d+\\s*:\\s*[A-Za-z]\\d+\\s*\\)$");
    }
}